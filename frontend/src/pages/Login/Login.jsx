...
const Login = ({ setShowLogin, defaultMode }) => {
  ...
  const sendOTPHandler = async () => {
    ...
    try {
      setError('');
      const res = await axios.post('http://localhost:5000/api/otp/send', { mobile: currState.mobile });
      if (res.data.success) {
        setOtpSent(true);
      } else {
  ...
  const verifyOTPHandler = async (e) => {
    ...
    try {
      setError('');
      const res = await axios.post('http://localhost:5000/api/otp/verify', { mobile: currState.mobile, otp });
      if (res.data.success) {
  ...
  const handleGoogleLogin = (credentialResponse) => {
    ...
    // Example: send credentialResponse.credential to backend
    setShowLogin && setShowLogin(false);
    navigate('/');
  };
  ...
  return (
    <div className="auth-page">
      <div className={`auth-form ${isDarkMode ? 'dark' : 'light'}`}> 
