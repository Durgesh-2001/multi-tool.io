...
const ImageGenerator = () => {
  ...
  const fetchAvailableOptions = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/imagegen/options`);
  ...
  const fetchFreeTools = async () => {
    try {
      const response = await axios.get(`${API_BASE_URL}/imagegen/free-tools`);
      if (response.data.success) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        ...
        window.dispatchEvent(new Event('authChange')); // Update balance
      } else {
      ...
      if (err.response) {
        if (err.response.status === 402) {
          ...
          errorMessage = 'You need more credits to generate an image.';
  ...
  const downloadImage = () => {
    ...
    try {
      ...
      link.href = `http://localhost:5000${generatedImage}`;
